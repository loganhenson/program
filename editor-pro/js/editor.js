  ...
  initialize(state, listen, emit) {
    ...
    window.vide.ports.requestSetupTerminalResizeObserver.subscribe(() => {
      ...
      const terminalResizeObserver = new ResizeObserver(_debounce(50, terminals => {
        ...
        // showing `?→` instead of `→`) when the PTY is sized down to 1
        // or 2 rows. Keeping the PTY at a sane minimum lets the shell
        // render normally even when the visible viewport is tiny.
